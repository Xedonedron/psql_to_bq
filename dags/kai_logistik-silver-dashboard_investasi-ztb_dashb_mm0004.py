...
TABLE = 'ztb_dashb_mm0004'
ALTER_TABLE = "data_account_assignment_po"
GCS_BUCKET = 'kai_sap'
...
def build_query_and_run(**kwargs):
    ...
    query = f"""
        INSERT INTO {PROJECT_ID}.{SCHEMA}.{ALTER_TABLE} (cl, purch_doc, item, saa, get_data_year, d, busa, g_l_acct, cost_ctr, `order`, asset, total_val_upon_release, funds_ctr, commitment_ltm, profit_ctr, prof_seg, nomor_produk_kai, name, wbs_element, wbs_element_2, last_update, last_insert_to_bigquery)
        SELECT
            CAST(mandt as STRING) as cl,
            CAST(ebeln as STRING) as purch_doc,
            CAST(ebelp as STRING) as item,
            CAST(zekkn as STRING) as saa,
...
with models.DAG(
    'dashboard_investasi-ztb_dashb_mm0004',
