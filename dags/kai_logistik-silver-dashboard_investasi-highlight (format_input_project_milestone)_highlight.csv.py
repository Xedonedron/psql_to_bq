...
TABLE = 'highlight (format_input_project_milestone)_highlight.csv'
...
def build_query_and_run(**kwargs):
    ...
    query = f"""
        CREATE or REPLACE TABLE {PROJECT_ID}.{SCHEMA}.highlight_format_input_project_milestone_highlight_csv as
        SELECT
            CAST(kendala_aksi_lanjut as STRING) as kendala_aksi_lanjut,
            CAST(support_bod as STRING) as support_bod,
            CAST(bulan as INTEGER) as bulan,
            CAST(tahun as INTEGER) as tahun,
            DATETIME('{current_ts}') AS last_insert_to_bigquery
        FROM bq_landing_zone.{SCHEMA}_highlight_format_input_project_milestone_highlight_csv;
    """
...
with models.DAG(
    'dashboard_investasi-highlight_format_input_project_milestone_highlight.csv',
    ...
    dump_from_postgres_to_gcs = PostgresToGCSOperator(
        ...
        sql=f"""
        SELECT
            "kendala_aksi_lanjut"::TEXT as "kendala_aksi_lanjut",
            "support_bod"::TEXT as "support_bod",
            "bulan"::INTEGER as "bulan",
    ...
    load_to_bigquery = GoogleCloudStorageToBigQueryOperator(
        ...
        destination_project_dataset_table=f'{PROJECT_ID}.bq_landing_zone.{SCHEMA}_highlight_format_input_project_milestone_highlight_csv', # Dataset harus dibuat terlebih dahulu
