...
TABLE = 'project_prognosa_forecast'
GCS_BUCKET = 'kai_sap'
...
def build_query_and_run(**kwargs):
    ...
    query = f"""
        ...
        SELECT
            ...
            CAST(program as NUMERIC) as program,
            CAST(realisasi as NUMERIC) as realisasi,
            CAST(prediction as NUMERIC) as prediction,
            CAST(prog_pred as NUMERIC) as prog_pred,
            CAST(latest_update as DATETIME) as latest_update,
...
with models.DAG(
    'dashboard_investasi-project_prognosa_forecast',
    ...
    dump_from_postgres_to_gcs = PostgresToGCSOperator(
        ...
        SELECT
            ...
            "program"::DOUBLE PRECISION as "program",
            "realisasi"::DOUBLE PRECISION as "realisasi",
            "prediction"::DOUBLE PRECISION as "prediction",
            "prog_pred"::DOUBLE PRECISION as "prog_pred",
