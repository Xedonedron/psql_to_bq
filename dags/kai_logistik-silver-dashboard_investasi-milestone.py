...
def build_query_and_run(**kwargs):
    ...
    query = f"""
        ...
        SELECT
            CAST(`Color Parameter_Line` as STRING) as `Color Parameter Line`,
            CAST(`Color Parameter_Shape` as STRING) as `Color Parameter Shape`,
            CAST(`Color Parameter_Size` as STRING) as `Color Parameter Size`,
            CAST(h0 as STRING) as h0,
            CAST(h1 as STRING) as h1,
            CAST(kategori as STRING) kategori,
            CAST(`tahun_copy` as INTEGER) as `tahun copy`,
            CAST(`Year Highlight` as BOOL) as `Year Highlight`,
            CAST(`+ Year` as INTEGER) as `+ Year`,
            CAST(`- Year` as INTEGER) as `- Year`,
            CAST(`Back Page` as BOOL) as `Back Page`,
            CAST(`Next Page` as BOOL) as `Next Page`,
            CAST(one as INTEGER) as one,
            CAST(tahun as INTEGER) as tahun,
            CAST(zero as INTEGER) as zero,
...
with models.DAG(
    ...
    dump_from_postgres_to_gcs = PostgresToGCSOperator(
        ...
        sql=f"""
        SELECT
            "Color Parameter (Line)"::TEXT as "Color Parameter_Line",
            "Color Parameter (Shape)"::TEXT as "Color Parameter_Shape",
            "Color Parameter (Size)"::TEXT as "Color Parameter_Size",
            "h0"::TEXT as "h0",
            "h1"::TEXT as "h1",
            "kategori"::TEXT as "kategori",
            "tahun (copy)"::INTEGER as "tahun_copy",
            "Year Highlight"::BOOLEAN as "Year Highlight",
            "+ Year"::INTEGER as "+ Year",
            "- Year"::INTEGER as "- Year",
            "Back Page"::BOOLEAN as "Back Page",
            "Next Page"::BOOLEAN as "Next Page",
            "one"::INTEGER as "one",
            "tahun"::INTEGER as "tahun",
            "zero"::INTEGER as "zero"
        FROM {SCHEMA}."{TABLE}";
        """,
        bucket=GCS_BUCKET,
